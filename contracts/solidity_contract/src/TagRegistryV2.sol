...
contract TagRegistryV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    // New feature in V2
    mapping(string => uint256) private tagCreatedAt;
    ...
    function registerTag(
        ...
        tagCreatedAt[tag] = block.timestamp;
    ...
    // New method in V2: Get tag creation timestamp
    function getTagCreatedAt(string memory tag) external view returns (uint256) {
        require(tags[tag].exists, "Tag not registered");
        return tagCreatedAt[tag];
    }
