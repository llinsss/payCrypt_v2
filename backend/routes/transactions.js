...
router.get(
  "/export/download",
  rateLimit({ endpointName: "download", windowMs: 15 * 60 * 1000, max: 10 }),
  downloadExport,
);
router.get(
  "/export",
  authenticateJwtOrApiKey,
  rateLimit({ endpointName: "api" }),
  rateLimit({ endpointName: "export", windowMs: 60 * 60 * 1000, max: 5 }),
  exportTransactions,
);
router.get(
  "/tag/:tag",
  authenticate,
  userRateLimiter,
  validateParams(transactionTagParamSchema),
...
router.put(
  ...
  authenticate,
  rateLimit({ endpointName: "api" }),
  rateLimit({ endpointName: "transactions" }),
  validateParams(transactionIdParamSchema),
