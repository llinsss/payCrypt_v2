...
/**
 * Validates that a date-of-birth string represents someone at least 18 years old.
 */
const minimumAge = (minYears) => (value, helpers) => {
...
const dobField = (required = true) => {
  const base = Joi.string()
    .isoDate()
    .custom(minimumAge(18))
    .messages({
  ...
  return required
    ? base.required().messages({ "any.required": "Date of birth is required" })
    : base.optional();
};
...
export const kycCreateSchema = Joi.object({
  ...
  id_image_url: Joi.string()
    .uri({ scheme: ["http", "https"] })
    .allow("", null)
    .optional()
    .messages({
      "string.uri": "ID image URL must be a valid http/https URL",
    }),

  selfie_url: Joi.string()
    .uri({ scheme: ["http", "https"] })
    .allow("", null)
    .optional()
    .messages({
      "string.uri": "Selfie URL must be a valid http/https URL",
    }),
});
...
export const kycUpdateSchema = Joi.object({
  first_name: Joi.string().trim().min(2).max(100).optional().messages({
    "string.min": "First name must be at least 2 characters",
    "string.max": "First name must be at most 100 characters",
  }),

  last_name: Joi.string().trim().min(2).max(100).optional().messages({
    "string.min": "Last name must be at least 2 characters",
    "string.max": "Last name must be at most 100 characters",
  }),
  ...
  id_number: Joi.string().trim().min(3).max(50).optional(),
